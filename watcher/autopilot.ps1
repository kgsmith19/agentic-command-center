# ACC autopilot loop (formerly "autopilot") - the outside process that physically types
...
#   6. Kill switch: watcher/autopilot.stop present => does nothing at all.
...
$StopFile = Join-Path $PSScriptRoot 'autopilot.stop'
$LogFile  = Join-Path $PSScriptRoot 'autopilot.log'
...
$HeartbeatFile = Join-Path $PSScriptRoot 'autopilot.heartbeat'
...
# dial's _note for why autopilot no longer guesses its own number here.
...
# whether autopilot should auto-replay a multi-line prompt is a separate,
...
# autopilot already runs continuously and independently of any one session's
# own hooks (unlike budget.mjs's reviveAutopilotIfDead, which only fires from
...
# memory, the same way autopilot.heartbeat already is: a `-Once` test run (or a
# crash-and-restart of autopilot itself) must not lose "was this alive a moment
...
Log "autopilot started (interval ${IntervalMs}ms, requests: $ReqDir)"
while ($true) {
