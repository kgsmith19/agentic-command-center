...
export function buildConsoleTable(win, { activeStanding, execFileSync, here, timeoutMs = 8000 }) {
  try {
    const pids = new Set();
    if (win && win.consolePid) pids.add(Number(win.consolePid));
    for (const g of activeStanding()) if (g.consolePid) pids.add(Number(g.consolePid));
    ...
    return undefined; // fail open: no table -> standing.mjs fails closed on its own
  }
}
