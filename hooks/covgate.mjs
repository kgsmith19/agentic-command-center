...
// Lib files only: .mjs directly under hooks/ or runner/, minus tests and
// harnesses. The GATE'S OWN SUBJECT, exported for its suite.
export function changedLibFiles(names) {
  return [...new Set(names)]
    .map((n) => String(n).replaceAll("\\", "/"))
    .filter((n) => /^(hooks|runner)\/[^/]+\.mjs$/.test(n) && !/\.(test|e2e)\.mjs$/.test(n));
}
