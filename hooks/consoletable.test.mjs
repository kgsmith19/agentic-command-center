...
// exercised in-process without real PowerShell or a real goal store - see the
...
test("no consolePid and no active goals -> undefined, never shells out", () => {
  let called = false;
...
test("gathers the window's own pid plus every active goal's consolePid, deduped", () => {
  let argsSeen = null;
  const result = buildConsoleTable(
    { consolePid: 111 },
    {
      activeGoals: () => [{ consolePid: 222 }, { consolePid: 111 }, { consolePid: 0 }, {}],
...
test("a goal with no consolePid at all contributes nothing to the pid set", () => {
  let called = false;
  const result = buildConsoleTable(null, {
