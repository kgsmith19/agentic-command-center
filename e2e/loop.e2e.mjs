...
//   ACC_ROOT   -> a throwaway runner tree (state, missions, clear-requests), so
...
function sandbox(hardK) {
  ...
  for (const d of [["runner", "state"], ["runner", "clear-requests"], ["runner", "missions"], ["watcher"]])
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      missions: { autoResume: true, maxCycles: 0, kickSettleSeconds: 5, humanHoldMinutes: 0 },
  ...
  // clearbot also shells to <root>/hooks/mission.mjs (pending kicks) and
  ...
  for (const f of ["mission.mjs", "usage.mjs"])
...
const node = (args, sb) =>
  execFileSync("node", args, {
    encoding: "utf8",
    env: { ...process.env, ACC_ROOT: sb.root, ACC_POLICY: sb.policyPath, ACC_MISSIONS_DIR: "" },
    windowsHide: true,
  });

function newMission(sb, text) {
  return JSON.parse(node([path.join(REPO, "hooks", "mission.mjs"), "new", "--text", text, "--cwd", REPO], sb));
}
...
// SessionStart fires before the TUI can accept input (the same reason mission.mjs
...
function startSession(sb, { missionId, prompt, cwd = REPO, extraEnv = {} }) {
  const wrapper = path.join(sb.root, `launch-${missionId || "x"}.cmd`);
  const sets = Object.entries({
    ACC_ROOT: sb.root,
    ACC_POLICY: sb.policyPath,
    ACC_MISSION: missionId || "",
    ACC_PROFILE: "",
    ...extraEnv,
  })
    .map(([k, v]) => `set "${k}=${v}"`)
...
function missionJson(sb, id) {
  try {
    return JSON.parse(fs.readFileSync(path.join(sb.root, "runner", "missions", `${id}.json`), "utf8"));
  } catch {
    return null;
  }
}
...
// session adopts the mission, clearbot types the resume constant, the cycle is
// logged. This is the whole promise in one run.
async function scenario1() {
  const sb = sandbox(5); // tiny ceiling: the first real turn is already over it
  const mission = newMission(sb, "Reply with exactly: BANANA. Then stop.");
  startSession(sb, { missionId: mission.id, prompt: "say BANANA and nothing else" });
  const consoles = [];
  try {
    const sid = waitFor("session binds to the mission", 180000, () => missionJson(sb, mission.id)?.sessionId);
    ...
    const cycled = cleared && waitFor("cycle logged", 60000, () => (missionJson(sb, mission.id)?.cycles || 0) >= 1);
    const newSid = cycled && waitFor("a NEW session adopts the mission", 180000, () => {
      const g = missionJson(sb, mission.id);
...
async function scenario2() {
  ...
  const mission = newMission(sb, "Reply with exactly: ok. Then stop.");
  startSession(sb, { missionId: mission.id, prompt: "say ok and nothing else" });
  const consoles = [];
  try {
    const sid = waitFor("session binds", 180000, () => missionJson(sb, mission.id)?.sessionId);
    const win = sid && waitFor("console pid recorded", 60000, () => windowOf(sb, sid)?.consolePid);
    if (win) consoles.push(win);
...
async function scenario3() {
  const sb = sandbox(5);
  const mission = newMission(sb, "Counting task.");
  startSession(sb, {
...
async function scenario4() {
  ...
  const mission = newMission(sb, "Directory move test.");
  const from = path.resolve(REPO, ".."); // C:\code - on the routes table
  startSession(sb, { missionId: mission.id, prompt: "say one and nothing else", cwd: from });
  const consoles = [];
  try {
    const sid = waitFor("session binds", 180000, () => missionJson(sb, mission.id)?.sessionId);
    const win = sid && waitFor("console pid recorded", 60000, () => windowOf(sb, sid)?.consolePid);
...
async function scenario5() {
  ...
  const mission = newMission(sb, "Reply with exactly: PTY. Then stop.");
  ...
  const host = spawn(
    ...
    ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", path.join(REPO, "gui", "ptyhost.e2e.ps1"),
     "-PipeName", pipeName, "-MissionId", mission.id, "-Cwd", REPO, "-PidFile", pidFile,
  ...
  try {
    ...
    const sid = childPid && waitFor("session binds to the mission", 180000, () => missionJson(sb, mission.id)?.sessionId);
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        try {
          ...
          if (kickAt < 0 && o.type === "user" &&
              JSON.stringify(o.message?.content ?? "").includes("Continue the active ACC mission.")) kickAt = i;
