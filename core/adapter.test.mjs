// node --test core/adapter.test.mjs  (run from the repo root)
...
// discuss a harness; CODE outside core/adapters/ may never name one.
//
...
test("no kernel module outside core/adapters/ references a harness (AC-A8)", () => {
  ...
  for (const f of files) {
    ...
    assert.doesNotMatch(code, /claude-code|codex|anthropic/i, `${f} names a harness outside core/adapters/`);
  }
});
