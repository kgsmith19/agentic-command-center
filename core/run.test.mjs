// node --test core/run.test.mjs  (run from the repo root)
...
// `node --test` import (the same shape core/ledger.mjs proves itself).
test("end-to-end: the CLI with no contract argument prints usage and exits 2", () => {
  assert.throws(
    ...
    (err) => /usage: node core\/run\.mjs/.test(err.stderr) && err.status === 2
  );
});
