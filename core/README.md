...
```
node core/run.mjs <contract.json>
```
...
A JSON file with these required fields (`core/contract.mjs`):
...
regardless of contract (`core/policy.mjs` `alwaysDenyWriteRoots`).

## Swapping the harness — step by step

1. Write `core/adapters/<name>.mjs` exporting exactly five members plus an
   id: `id`, `identity()`, `startTask()`, `sendStep()`, `readState()`,
   `stopTask()` — the shape `core/adapter.mjs`'s `ADAPTER_INTERFACE` checks.
2. Set `policy.json`'s `kernel.harness` to `<name>`.
3. Run `node --test core/adapter.test.mjs` — the shape check and the
   isolation test are what prove the swap needs no other change.

No file outside `core/adapters/` mentions a harness by name. The module
...
kernel guardhook (`core/guardhook.mjs`, registered on every tool that does
...
from `core/run.mjs`'s supervisor tick) never reads the harness child's
...
`core/run.test.mjs`'s AC-B1 test proves exactly this shape: a fake adapter
...
The contract lists vault key **names**; `core/credentials.mjs` is the only
...
```
node core/ledger.mjs query [--status <s>] [--harness <h>] [--since <date>] [--until <date>]
```
