// node --test core/settings.test.mjs  (run from the repo root)
...
test("the guardhook matcher is exactly the contract's tool allowlist", () => {
  const s = S.generateSettings(contract, { guardhookPath: "C:/g/core/guardhook.mjs" });
...
test("writeRunFiles pins the settings hash and stores the contract for the hook", () => {
  const w = S.writeRunFiles(contract, { runId: "r-pin", guardhookPath: "C:/g/core/guardhook.mjs" });
...
test("a TAMPERED settings file fails the integrity check (AC-G5, AC-G6)", () => {
  const w = S.writeRunFiles(contract, { runId: "r-tamper", guardhookPath: "C:/g/core/guardhook.mjs" });
...
test("a missing settings file or pin fails closed, never passes by default", () => {
  const w = S.writeRunFiles(contract, { runId: "r-gone", guardhookPath: "C:/g/core/guardhook.mjs" });
...
test("a corrupt (not valid JSON) pin.json fails closed too, distinct from a missing one (fault-tolerance)", () => {
  const w = S.writeRunFiles(contract, { runId: "r-corrupt-pin", guardhookPath: "C:/g/core/guardhook.mjs" });
...
test("a contract granting no optional actions still produces a valid matcher of just the always-allowed tools (edge)", () => {
  ...
  const s = S.generateSettings(bare, { guardhookPath: "C:/g/core/guardhook.mjs" });
  assert.equal(s.hooks.PreToolUse[0].matcher, "TodoWrite");
});

test("cleanupRun removes the staging directory", () => {
  const w = S.writeRunFiles(contract, { runId: "r-clean", guardhookPath: "C:/g/core/guardhook.mjs" });
