...
2. **Self-protection** — currently **OFF**: `C:/code/guards` is not in the
   ...
   the ACC mission closes, it should be re-added (`C:/code/guards/` in full, or
...
accepted (autoApprove stays on for the throughput it buys the unattended mission
...
interactive ConPTY/mission-loop path above. See `kernel/README.md` for the
...
```
node --test hooks/budget.test.mjs hooks/mission.test.mjs hooks/usage.test.mjs hooks/route.test.mjs hooks/statusline.test.mjs hooks/clearbot.test.mjs hooks/lane.test.mjs hooks/testplan.test.mjs hooks/covgate.test.mjs hooks/prompts.test.mjs hooks/cmdline.test.mjs runner/runner.test.mjs kernel/adapter.test.mjs kernel/adapters/claude-code.test.mjs kernel/autonomy.test.mjs kernel/contract.test.mjs kernel/credentials.test.mjs kernel/guard.test.mjs kernel/guardhook.test.mjs kernel/ledger.test.mjs kernel/policy.test.mjs kernel/run.test.mjs kernel/settings.test.mjs kernel/verifier.test.mjs gui/server.test.mjs gui/guards-gui.test.mjs
...
node kernel/kernel.e2e.mjs
    -> PROOF TIER. Spawns a REAL claude twice via kernel/run.mjs and spends
       ...
       rejected; a third check confirms no ACC mission-loop state leaks from a
...
**Never run a hook by hand against live state.** `bindSession` adopts a mission by
console PID, so piping a fake SessionStart into `budget.mjs` from a console
that owns a mission used to rebind that mission to whatever session id the payload
...
board task), the proof tier, and the mission loop all open real API streams, and
...
would stall the mission loop, which has no replay for it) injects the contract
...
## Missions — how a session survives its own context limit

A **mission** is a piece of work that outlives the session doing it. The GUI's GO
button creates one (`hooks/mission.mjs new --text-file`) and launches Claude with
`ACC_MISSION=<id>`; from then on the loop runs with no human in it:
...
the mission and injects it → clearbot types `Continue the active ACC mission.`
...
1. **A mission binds to the CONSOLE PID, not the session id.** A `/clear` ends the
   session id; the terminal process is the same throughout. Every session that
   starts in that console adopts the mission, which is what makes resumption survive
   the clear. Queued prompts (above) are keyed the same way for the same reason.
2. **Mission text never becomes keystrokes.** It reaches the model through
...
State: `runner\missions\<id>.json` plus a running `<id>.log.md`, archived to
`runner\missions\done\` on completion. **The loop only ends because the model ends
it** — `mission.mjs done <id>` or `mission.mjs blocked <id> --why "..."`, both stated
in full in the injected block. The week kill switch is the cost brake; a red week
holds all kicks. `mission.mjs pending` decides every condition that makes a kick
...
Tests: `node --test hooks/mission.test.mjs` (48).

Two things keep the loop from stalling, added 2026-07-31 after it stalled
twice in one day. **Liveness:** a mission session that ends its turn UNDER the
ceiling gets no clear, so the Stop hook re-arms the kick instead
(`mission.mjs recordTurnEnd`), and `pendingKicks` decides when firing it is safe —
after `missions.kickSettleSeconds` (90), and not within `missions.humanHoldMinutes`
...
`/mission <condition>` (user skill `~\.claude\skills\mission\`) is ACC-native: it
logs `CONDITION: <text>` into the active mission's log via `mission.mjs log`, so the
directive rides the mission store and survives every `/clear` with the rest of
the handoff; `/mission clear` logs `CONDITION MET`. It never registers a session
Stop hook: a Stop-gate fights the budget gate (OI-011) — the loop continues BY
ending turns — so conditions live in mission state, not hooks. With no active
mission the condition is session-local only.
