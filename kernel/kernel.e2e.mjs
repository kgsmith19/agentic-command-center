...
// ------------------------------------------------------------- pollution check
// Neither scenario is allowed to leave a mark on the live ACC mission-loop
// state this kernel effort shares a repo with. A user-level hook firing
// inside a kernel run would show up here — if it does, this is NOT fixed
// silently (plan Task 19 step 4): it is ledgered and reported instead.
function checkNoPollution(before) {
  const after = listSafe(path.join(REPO, "runner", "missions"));
  const pass = JSON.stringify(before) === JSON.stringify(after);
  report("pollution", "no new file appears under the LIVE runner/missions/ during kernel runs", pass,
    `before: ${JSON.stringify(before)}\nafter:  ${JSON.stringify(after)}`);
}

const liveMissionsBefore = listSafe(path.join(REPO, "runner", "missions"));
await scenario1();
await scenario2();
checkNoPollution(liveMissionsBefore);
