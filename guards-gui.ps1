...
# Multi-line on purpose: this box is the DIRECTIVE, and a directive worth resuming rarely
# fits on one line. It never becomes keystrokes (see hooks/directive.mjs), so newlines
# in here are safe.
...
# two ways to end it.
$grpDirective = Add-Ctl $tabS (New-Object System.Windows.Forms.GroupBox) 15 504 650 96
$grpDirective.Text = ' What Claude is working on now '
$grpDirective.Font = New-Object System.Drawing.Font('Segoe UI', 10, [System.Drawing.FontStyle]::Bold)
$lblDirective = Add-Ctl $grpDirective (New-Object System.Windows.Forms.Label) 15 24 615 26
$lblDirective.Font = New-Object System.Drawing.Font('Segoe UI', 9)
$btnDirectiveDone = Add-Ctl $grpDirective (New-Object System.Windows.Forms.Button) 15 54 200 28
$btnDirectiveDone.Text = 'Mark it finished'
$btnDirectiveStop = Add-Ctl $grpDirective (New-Object System.Windows.Forms.Button) 225 54 200 28
$btnDirectiveStop.Text = 'Stop restarting it'
$btnDirectiveLog = Add-Ctl $grpDirective (New-Object System.Windows.Forms.Button) 435 54 195 28
$btnDirectiveLog.Text = 'Open the progress log'
...
$script:DirectiveJs = Join-Path $PSScriptRoot 'hooks\directive.mjs'
$script:DirectiveId = ''

# Every Go creates a directive, so every session ACC starts is one it can resume.
...
function New-DirectiveFromBox([string]$Text, [string]$Dir, [string]$Profile) {
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("acc-directive-" + [Guid]::NewGuid().ToString('N') + '.txt')
    try {
        [System.IO.File]::WriteAllText($tmp, $Text, (New-Object System.Text.UTF8Encoding($false)))
        $r = Invoke-Node $script:DirectiveJs @('new', '--text-file', $tmp, '--cwd', $Dir, '--profile', $Profile)
        if ($r.ExitCode -ne 0) { return $null }
        return ($r.Out.Trim() | ConvertFrom-Json)
    } catch { return $null } finally { Remove-Item $tmp -ErrorAction SilentlyContinue }
}

function Refresh-Directives {
    $directives = @()
    try {
        $r = Invoke-Node $script:DirectiveJs @('list')
        if ($r.ExitCode -eq 0) { $directives = @(ConvertFrom-JsonArray ($r.Out.Trim())) }
    } catch { }
    $g = $null
    if ($script:DirectiveId) { $g = $directives | Where-Object { $_.id -eq $script:DirectiveId } | Select-Object -First 1 }
    if (-not $g) { $g = $directives | Select-Object -Last 1 }
    if (-not $g) {
        $script:DirectiveId = ''
        $lblDirective.ForeColor = [System.Drawing.Color]::FromArgb(90, 95, 100)
        $lblDirective.Text = 'Nothing running. Type the job above and press Go.'
        $btnDirectiveDone.Enabled = $false; $btnDirectiveStop.Enabled = $false; $btnDirectiveLog.Enabled = $false
        return
    }
    $script:DirectiveId = $g.id
    ...
    $lblDirective.ForeColor = [System.Drawing.Color]::FromArgb(20, 90, 40)
    $lblDirective.Text = ("Working: {0}  ({1})" -f $first, $when)
    $btnDirectiveDone.Enabled = $true; $btnDirectiveStop.Enabled = $true; $btnDirectiveLog.Enabled = $true
}

$btnDirectiveDone.Add_Click({
    if (-not $script:DirectiveId) { return }
    [void](Invoke-Node $script:DirectiveJs @('done', $script:DirectiveId, '--why', 'marked finished from the Command Center'))
    $script:DirectiveId = ''
    Refresh-Directives
})
$btnDirectiveStop.Add_Click({
    if (-not $script:DirectiveId) { return }
    [void](Invoke-Node $script:DirectiveJs @('paused', $script:DirectiveId))
    $script:DirectiveId = ''
    Refresh-Directives
})
$btnDirectiveLog.Add_Click({
    if (-not $script:DirectiveId) { return }
    $p = Join-Path $PSScriptRoot ("runner\directives\{0}.log.md" -f $script:DirectiveId)
...
$btnStartWork.Add_Click({
    ...
    $name = Get-SelectedProfile
    $directive = New-DirectiveFromBox $task $dir $name
    if (-not $directive) {
        $lblStartOut.ForeColor = [System.Drawing.Color]::Firebrick
        $lblStartOut.Text = 'Could not save the directive - not starting. (Is node on PATH?)'
        return
    }
    try {
        if ($script:TermOk -and $script:wv -and $script:wv.CoreWebView2) {
            ...
            if (-not (Start-PtySession -DirectiveId $directive.id -ProfileName $name -Dir $dir)) { return }
        } else {
            ...
            # SessionStart reads this, binds the directive to that console, and injects the
            # text. It is the reason the work survives every later /clear.
            $psi.EnvironmentVariables['ACC_DIRECTIVE'] = $directive.id
            try {
                $p = [System.Diagnostics.Process]::Start($psi)
        ...
        $script:DirectiveId = $directive.id
...
if ($script:TermOk) {
    ...
    $btnTermStart = New-DeckButton 'Start'    {
        if ($script:lastStart) {
            [void](Start-PtySession -DirectiveId $script:lastStart.directive -ProfileName $script:lastStart.profile -Dir $script:lastStart.dir)
        }
    }
...
function Start-PtySession([string]$DirectiveId, [string]$ProfileName, [string]$Dir) {
    if ($script:pty) {
        $a = [System.Windows.Forms.MessageBox]::Show(
    ...
    # restore. SessionStart reads ACC_DIRECTIVE to bind the directive and ACC_PTY to
    # record transport:"pty" + the pipe name.
    $env:ACC_DIRECTIVE = $DirectiveId; $env:ACC_PROFILE = $ProfileName; $env:ACC_PTY = $pipeName
    try {
        ...
        Remove-Item Env:ACC_DIRECTIVE, Env:ACC_PROFILE, Env:ACC_PTY -ErrorAction SilentlyContinue
    }
    $script:lastStart = @{ directive = $DirectiveId; profile = $ProfileName; dir = $Dir }
    if (Get-Command Update-Deck -ErrorAction SilentlyContinue) { Update-Deck }
    return $true
}
...
# Status strip refresh: directive id, transport/pipe/binding state, child pid,
...
$script:deckTimer.Add_Tick({
    ...
    $parts = @()
    if ($script:DirectiveId) { $parts += ('directive ' + $script:DirectiveId) }
    if ($script:pty) {
...
$pnlWork.Dock = 'Fill'
foreach ($c in @($lblS0, $lblS0b, $grpS1, $grpS2, $btnStartWork, $lblStartOut, $grpDirective, $lblS3)) {
    $at = $c.Location
...
# The directive line has to move on its own: the thing that advances it is a session
# in another window, and Kyle is not going to press Refresh to find that out.
$directiveTimer = New-Object System.Windows.Forms.Timer
$directiveTimer.Interval = 5000
$directiveTimer.Add_Tick({ if ($pnlWork.Visible) { Refresh-Directives } })
$form.Add_Shown({ $directiveTimer.Start() })
$form.Add_FormClosed({
    $directiveTimer.Stop(); $directiveTimer.Dispose()
