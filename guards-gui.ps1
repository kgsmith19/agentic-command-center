...
# Multi-line on purpose: this box is the MISSION, and a mission worth resuming rarely
# fits on one line. It never becomes keystrokes (see hooks/mission.mjs), so newlines
# in here are safe.
...
$script:MissionJs = Join-Path $PSScriptRoot 'hooks\mission.mjs'
$script:MissionId = ''

# Every Go creates a mission, so every session ACC starts is one it can resume.
...
function New-MissionFromBox([string]$Text, [string]$Dir, [string]$Profile) {
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("acc-mission-" + [Guid]::NewGuid().ToString('N') + '.txt')
    try {
        [System.IO.File]::WriteAllText($tmp, $Text, (New-Object System.Text.UTF8Encoding($false)))
        $r = Invoke-Node $script:MissionJs @('new', '--text-file', $tmp, '--cwd', $Dir, '--profile', $Profile)
        if ($r.ExitCode -ne 0) { return $null }
        return ($r.Out.Trim() | ConvertFrom-Json)
    } catch { return $null } finally { Remove-Item $tmp -ErrorAction SilentlyContinue }
}

function Refresh-Missions {
    $missions = @()
    try {
        $r = Invoke-Node $script:MissionJs @('list')
        if ($r.ExitCode -eq 0) { $missions = @(ConvertFrom-JsonArray ($r.Out.Trim())) }
    } catch { }
    $g = $null
    if ($script:MissionId) { $g = $missions | Where-Object { $_.id -eq $script:MissionId } | Select-Object -First 1 }
    if (-not $g) { $g = $missions | Select-Object -Last 1 }
    if (-not $g) {
        $script:MissionId = ''
...
$btnStartWork.Add_Click({
    ...
    $name = Get-SelectedProfile
    $mission = New-MissionFromBox $task $dir $name
    if (-not $mission) {
        $lblStartOut.ForeColor = [System.Drawing.Color]::Firebrick
        $lblStartOut.Text = 'Could not save the mission - not starting. (Is node on PATH?)'
        return
    }
    try {
        if ($script:TermOk -and $script:wv -and $script:wv.CoreWebView2) {
            ...
            if (-not (Start-PtySession -MissionId $mission.id -ProfileName $name -Dir $dir)) { return }
        } else {
            ...
            # SessionStart reads this, binds the mission to that console, and injects the
            # text. It is the reason the work survives every later /clear.
            $psi.EnvironmentVariables['ACC_MISSION'] = $mission.id
            try {
                $p = [System.Diagnostics.Process]::Start($psi)
...
if ($script:TermOk) {
    ...
    $btnTermStart = New-DeckButton 'Start'    {
        if ($script:lastStart) {
            [void](Start-PtySession -MissionId $script:lastStart.mission -ProfileName $script:lastStart.profile -Dir $script:lastStart.dir)
        }
    }
...
function Start-PtySession([string]$MissionId, [string]$ProfileName, [string]$Dir) {
    if ($script:pty) {
        $a = [System.Windows.Forms.MessageBox]::Show(
    ...
    # restore. SessionStart reads ACC_MISSION to bind the mission and ACC_PTY to
    # record transport:"pty" + the pipe name.
    $env:ACC_MISSION = $MissionId; $env:ACC_PROFILE = $ProfileName; $env:ACC_PTY = $pipeName
    try {
        ...
        Remove-Item Env:ACC_MISSION, Env:ACC_PROFILE, Env:ACC_PTY -ErrorAction SilentlyContinue
    }
    $script:lastStart = @{ mission = $MissionId; profile = $ProfileName; dir = $Dir }
    if (Get-Command Update-Deck -ErrorAction SilentlyContinue) { Update-Deck }
    return $true
}
...
# Status strip refresh: mission id, transport/pipe/binding state, child pid,
...
$script:deckTimer.Add_Tick({
    ...
    $parts = @()
    if ($script:MissionId) { $parts += ('mission ' + $script:MissionId) }
    if ($script:pty) {
...
# The mission line has to move on its own: the thing that advances it is a session
# in another window, and Kyle is not going to press Refresh to find that out.
$missionTimer = New-Object System.Windows.Forms.Timer
$missionTimer.Interval = 5000
$missionTimer.Add_Tick({ if ($pnlWork.Visible) { Refresh-Missions } })
$form.Add_Shown({ $missionTimer.Start() })
$form.Add_FormClosed({
    $missionTimer.Stop(); $missionTimer.Dispose()
